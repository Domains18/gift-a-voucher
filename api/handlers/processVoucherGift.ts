            ...
            try {
                ...
                if (message.voucherId) {
                    if (receiveCount >= MAX_RETRY_COUNT) {
                        Logger.info(
                            'PROCESSOR',
                            `Updating voucher ${message.voucherId} status to FAILED after max retries`,
                        );
                        await DynamoDBService.updateVoucherStatus(message.voucherId, 'FAILED');
                        Logger.info(
                            'PROCESSOR',
                            `Updated voucher ${message.voucherId} status to FAILED after max retries`,
                        );
                    } else if (shouldRetry) {
                        ...
                        Logger.info(
                            'PROCESSOR',
                            `Updating voucher ${message.voucherId} status to FAILED due to non-retryable error`,
                        );
                        await DynamoDBService.updateVoucherStatus(message.voucherId, 'FAILED');
                        Logger.info(
                            'PROCESSOR',
                            `Updated voucher ${message.voucherId} status to FAILED due to non-retryable error`,
                        );
                    }
                }
            } catch (parseError) {
